...
std::string webPedalJson() {
    ...
    json += "\",\"position_P\":\"";
    json += std::to_string(pedalParams.position_P);
    json += "\",\"position_I\":\"";
    json += std::to_string(pedalParams.position_I);
    json += "\",\"position_D\":\"";
    json += std::to_string(pedalParams.position_D);
    json += "\",\"ramp\":\"";
    json += std::to_string(pedalParams.ramp);
    json += "\",\"limit\":\"";
    json += std::to_string(pedalParams.limit);
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "position_P", param, sizeof(param)) == ESP_OK) {
                pedalParams.position_P = std::stof(param);
            }
            if (httpd_query_key_value(buf, "position_I", param, sizeof(param)) == ESP_OK) {
                pedalParams.position_I = std::stof(param);
            }
            if (httpd_query_key_value(buf, "position_D", param, sizeof(param)) == ESP_OK) {
                pedalParams.position_D = std::stof(param);
            }
            if (httpd_query_key_value(buf, "ramp", param, sizeof(param)) == ESP_OK) {
                pedalParams.ramp = std::stof(param);
            }
            if (httpd_query_key_value(buf, "limit", param, sizeof(param)) == ESP_OK) {
                pedalParams.limit = std::stof(param);
            }

            savePedalParams();

            ThrottleControl::set_params(
                pedalParams.position_P, 
                pedalParams.position_I, 
                pedalParams.position_D, 
                pedalParams.ramp, 
                pedalParams.limit
            );
        }
